#!/usr/bin/env python3
"""
Antigravity IDE/CLI log scanner — extracts the exact user-typed prompts from
...
Why not other sources we considered?
  - Antigravity conversation protobuf files are encrypted.
...
# Current CLI and IDE locations first; keep the original IDE path as fallback.
BRAIN_CANDIDATES = (
...
def git(*args: str) -> str:
    try:
        return subprocess.check_output(
            ["git", *args], cwd=REPO_ROOT, shell=False, text=True, stderr=subprocess.DEVNULL
        ).strip()
    except Exception:
        return ""
...
def _normalize(p: str) -> str:
    """Normalize a path using the current operating system's rules."""
    if not p:
        return ""
...
def _conv_matches_repo(cwds: set[str], repo_root_n: str) -> bool:
    ...
    for cwd in cwds:
        try:
            if os.path.commonpath((cwd, repo_root_n)) in (cwd, repo_root_n):
                return True
        except ValueError:
            continue
    return False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract user prompts from Antigravity IDE/CLI transcripts"
    ...
    print(f"[antigravity-log] Logged {len(new_entries)} prompt(s) from Antigravity IDE/CLI.", file=sys.stderr)
