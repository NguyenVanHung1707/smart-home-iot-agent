...
### Các AI tools được hỗ trợ tự động
...
| **OpenAI Codex** | `.codex/config.toml` | Mỗi prompt + khi stop |
| **GitHub Copilot** | `.github/hooks/hooks.json` | Mỗi prompt + khi session end |
| **Antigravity CLI/IDE** | `.agents/hooks.json` + pre-push | Quét transcript khi stop và `git push` |
...
Chỉ dùng cho ChatGPT, Claude.ai, Gemini Web hoặc tool không có lifecycle hook:

```bash
bash scripts/_pyrun.sh scripts/log_manual.py --tool chatgpt --prompt "Brainstorm UI layout"
```

Trên Windows dùng `scripts\_pyrun.cmd` với cùng tham số. Không chạy thủ công cho
tool đã có hook vì sẽ tạo log trùng.
...
| Antigravity không log | Chưa có transcript | Chắc chắn dùng Antigravity IDE/CLI trong repo folder |
